...
export default function TabLayout() {
  const colors = useColors(); const dark = useColorScheme() === 'dark'; const ios = Platform.OS === 'ios'; const web = Platform.OS === 'web'; const insets = useSafeAreaInsets();
  const icon = (name: keyof typeof Feather.glyphMap) => ({ color }: { color: string }) => <Feather name={name} size={21} color={color} />;
  return <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: colors.primary, tabBarInactiveTintColor: colors.mutedForeground, tabBarStyle: { position: 'absolute', backgroundColor: ios ? 'transparent' : colors.background, borderTopWidth: web ? 1 : 0, borderTopColor: colors.border, elevation: 0, height: web ? 84 : 68 + insets.bottom, paddingBottom: web ? 0 : insets.bottom, paddingTop: web ? 0 : 8 }, tabBarBackground: () => ios ? <BlurView intensity={90} tint={dark ? 'dark' : 'light'} style={StyleSheet.absoluteFill} /> : <View style={[StyleSheet.absoluteFill, { backgroundColor: colors.background }]} /> }}>
